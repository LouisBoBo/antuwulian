...
public class GpsUtils {
    ...
    public static boolean isChinaLocation(double lat, double lon) {
        ...
        int mapType = ConstantValue.getMapType(); //地图类型，0：百度地图，1：高德地图，2：谷歌地图
        if (mapType == 0) {// 百度地图、使用的国内边界线
            isChina = SpatialRelationUtil.isPolygonContainsPoint(PolygonalArea.getBaiduListPoint(), latLng);
        } else { //高德地图 、谷歌地图使用的中国边界
            isChina = SpatialRelationUtil.isPolygonContainsPoint(PolygonalArea.getBaiduGListPoint(), latLng);
        }
        return isChina;
    }
    ...
    public static boolean isChinaTaiWan(double lat, double lng) {
        ...
        isTaiwan = SpatialRelationUtil.isPolygonContainsPoint(PolygonalArea.getTaiWanListPoint(), latLng);
        return isTaiwan;
    }
    ...
    public static boolean isChinaHangKong(double lat, double lng) {
        ...
        com.baidu.mapapi.model.LatLng latLng = new com.baidu.mapapi.model.LatLng(lat, lng);
        isHangKong = SpatialRelationUtil.isPolygonContainsPoint(PolygonalArea.getHangKongListPoint(), latLng);
        return isHangKong;
    }
     ...
     * @return
     */
    public static boolean isChinaMaCao(double lat, double lng) {
        // 判断当前点的经纬度是否在中国范围内的经纬度多边形内，判断国内和海外
        boolean isAoMen; // 是否在澳门
        com.baidu.mapapi.model.LatLng latLng = new com.baidu.mapapi.model.LatLng(lat, lng);
        isAoMen = SpatialRelationUtil.isPolygonContainsPoint(PolygonalArea.getAoMenListPoint(), latLng);
        return isAoMen;
    }
    ...
    public static int PointType(double lat, double lng) {
        ...
        if (!isChina) { // 海外
            ...
            if (isChinaTaiWan(lat, lng)) { //中国台湾
                ...
                return 3;
            } else if (isChinaMaCao(lat, lng)) { //中国澳门
                return 4;
            } else {
                return 0;
            }
    ...
    public static com.google.android.gms.maps.model.LatLng googleGPSConverter(double lat, double lng) {
        int type = PointType(lat,lng);
        if (type != 0) { // 海外、包含香港、澳门、台湾
     ...
     * @return
     */
    public static com.amap.api.maps.model.LatLng aMapGPSConverter(double lat, double lng) {
        int type = PointType(lat, lng);
        if (type == 1 || type == 2) { //1 台湾   2 国外
            double[] latLon = GpsUtils.toWGS84Point(lat, lng);
            return new com.amap.api.maps.model.LatLng(latLon[0], latLon[1]);
        } else {
     ...
     * @return
     */
    public static com.baidu.mapapi.model.LatLng baiduGPSConverter(double lat, double lng) {
        int type = PointType(lat, lng);
        if (type == 2) { //海外
            double[] latLon = GpsUtils.toWGS84Point(lat, lng);
            return new com.baidu.mapapi.model.LatLng(latLon[0], latLon[1]);
        } else {
            return new com.baidu.mapapi.model.LatLng(lat, lng);
        }
    }
