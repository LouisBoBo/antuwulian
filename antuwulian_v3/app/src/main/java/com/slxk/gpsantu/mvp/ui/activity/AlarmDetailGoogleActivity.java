...
public class AlarmDetailGoogleActivity extends BaseActivity<AlarmDetailGooglePresenter> implements AlarmDetailGoogleContract.View , OnMapReadyCallback {
    ...
    public void setupActivityComponent(@NonNull AppComponent appComponent) {
        DaggerAlarmDetailGoogleComponent //如找不到该类,请编译一下项目
                .builder()
    ...
    public int initView(@Nullable Bundle savedInstanceState) {
        return R.layout.activity_alarm_detail_google; //如果你不需要框架帮你设置 setContentView(id) 需要自行设置,请返回 0
    }
    ...
    public void onMapReady(@NonNull GoogleMap googleMap) {
        googleMap.getUiSettings().setRotateGesturesEnabled(false);//选择手势
        googleMap.getUiSettings().setZoomControlsEnabled(false);//缩放按钮
