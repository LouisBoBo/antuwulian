...
public class RecordActivity extends BaseActivity<RecordPresenter> implements RecordContract.View {
    ...
    private MyDao recordTimeModelDao;
    ...
    public void initData(@Nullable Bundle savedInstanceState) {
        ...
        tvCancel.setText(getString(R.string.txt_delete));
        tvCancel.setTextColor(getResources().getColor(R.color.color_676767));
        ...
        try {
            recordBeanDao = new MyDao(RecordModel.class);
            recordTimeModelDao = new MyDao(RecordTimeModel.class);
    ...
    public List<RecordTimeModel> getRecordTimeDataBase(String imei) {
        try {
            ...
            return (List<RecordTimeModel>) recordTimeModelDao.query(recordTimeModelDao.queryBuilder(map1));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public void onDeleteRecordTimeForDB(RecordTimeModel recordTimeModel) {
        try {
            recordTimeModelDao.delete(recordTimeModel);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
     ...
     * @param recordModels
     */
    public void onDeleteRecordForDB(List<RecordModel> recordModels) {
        try {
            recordBeanDao.delete(recordModels);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
        ...
        if (Utils.isButtonQuickClick(System.currentTimeMillis())) {
            switch (view.getId()) {
                case R.id.toolbar_right: // 取消删除
                    ...
                    tvCancel.setText(isEditDelete ? getString(R.string.txt_cancel) : getString(R.string.txt_delete));
                    if(!isEditDelete){
                        tvSelectAll.setText(getString(R.string.txt_select_all));
                        isAllSelect = false;
                    }
    ...
    public void getRecordDataSuccess(RecordResultBean recordResultBean, boolean isHistoryData) {
        ...
        if (isHistoryData) {
            ...
            if (listFile.size() > 0) {
                ...
                historyRecordDownload = new RecordDownload(this, mImei, handlerCallbackForHistory, recordBeanDao, recordTimeModelDao);
            ...
            if (listFile.size() > 0) {
                ...
                realTimeRecordDownload = new RecordDownload(this, mImei, handlerCallback,recordBeanDao,recordTimeModelDao);
