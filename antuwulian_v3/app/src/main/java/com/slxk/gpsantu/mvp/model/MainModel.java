...
public class MainModel extends BaseModel implements MainContract.Model {
    ...
    @Override
    public Observable<AreaFileResultBean> getAreaFile(AreaFilePutBean bean) {
        Gson gson = new Gson();
        RequestBody requestBody = RequestBody.create(MediaType.parse("application/json"), gson.toJson(bean));
        return Observable.just(mRepositoryManager.obtainRetrofitService(PublicService.class).getAreaFile(requestBody))
                .flatMap(new Function<Observable<AreaFileResultBean>, ObservableSource<AreaFileResultBean>>() {
                    @Override
                    public ObservableSource<AreaFileResultBean> apply(Observable<AreaFileResultBean> languageFileResultBeanObservable) throws Exception {
                        return mRepositoryManager.obtainRetrofitService(PublicService.class).getAreaFile(requestBody);
                    }
                });
    }

    @Override
    public Observable<AreaResultBean> getArea(String url) {
        return Observable.just(mRepositoryManager.obtainRetrofitService(PublicService.class).getArea(Api.APP_DOMAIN + url))
                .flatMap(new Function<Observable<AreaResultBean>, ObservableSource<AreaResultBean>>() {
                    @Override
                    public ObservableSource<AreaResultBean> apply(Observable<AreaResultBean> languageResultBeanObservable) throws Exception {
                        return mRepositoryManager.obtainRetrofitService(PublicService.class).getArea(Api.APP_DOMAIN + url);
                    }
                });
    }
}