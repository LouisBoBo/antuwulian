...
import com.slxk.gpsantu.di.component.DaggerTrackGoogleComponent;
import com.slxk.gpsantu.mvp.contract.TrackGoogleContract;
import com.slxk.gpsantu.mvp.model.api.ModuleValueService;
import com.slxk.gpsantu.mvp.model.bean.AlertBean;
import com.slxk.gpsantu.mvp.model.bean.RoutePointGoogle;
...
import com.slxk.gpsantu.mvp.presenter.TrackGooglePresenter;
...
public class TrackGoogleActivity extends BaseActivity<TrackGooglePresenter> implements TrackGoogleContract.View,
        OnMapReadyCallback, GoogleMap.OnMarkerClickListener, GoogleMap.OnCameraChangeListener {
    ...
    private LatLng targetLatLng = new LatLng(39.90923, 116.397428);
    ...
    private Marker currentMarker, qiMarket, zhongMarket, playCurrrentMarker;//当前marker   起点   终点  停车点 播放点
    ...
    private float mPlayRotate; // 播放轨迹旋转角度
    private LatLng lastPoint; // 上一个点的经纬度 - 播放轨迹
    private LatLng currentPoint; // 当前点的经纬度 - 播放轨迹
    ...
    public static Intent newInstance() {
        return new Intent(MyApplication.getMyApp(), TrackGoogleActivity.class);
    }
...
//        setDataForTrack("", "");
    ...
    public void onMapReady(@NonNull GoogleMap googleMap) {
        ...
        mGoogleMap.getUiSettings().setRotateGesturesEnabled(false); //禁用地图手势旋转
        mGoogleMap.setOnMarkerClickListener(this);
        mGoogleMap.setOnCameraChangeListener(this);

        mGoogleMap.animateCamera(CameraUpdateFactory.newCameraPosition(new CameraPosition(carLocation, mZoom, 0, 0)));
        bitmapHelper = new BitmapHelperGoogle(this);

        bitmapDescriptor = BitmapDescriptorFactory.fromBitmap(Utils.decodeSampledBitmapFromResource(getResources(),
...
//        getTrackHasForData();   隐藏获取有轨迹的点
    ...
    public boolean onMarkerClick(@NonNull Marker marker) {
        if (playCurrrentMarker != null && playCurrrentMarker.getId().equals(marker.getId())) {
            return true;
        }
        try {
            infoWindow(marker);
    ...
    @SuppressLint("SetTextI18n")
    private void infoWindow(@NonNull Marker marker){
        ...
        mapInfoWindow.tvMarkerAddress.setText(getString(R.string.txt_address)); // 先置空初始化
        ...
        if (detailInfo.length == 9) {
            ...
            new LocationAddress().Parsed(selectLatForPoint, selectLonForPoint)
                    .setAddressListener(str -> {
                        ...
                        showInfoWindow(mapInfoWindow.viewInfoWindows);
                    });
    ...
    // 展示InfoWindow
    private void showInfoWindow(View infoView){
    ...
    @OnClick({R.id.iv_play, R.id.iv_play_speed, R.id.tv_play_speed, R.id.ll_base_station, R.id.tv_before_day,
            R.id.tv_after_day, R.id.toolbar_iv_right,R.id.ll_start_end_time, R.id.iv_map_check, R.id.ll_wifi_station})
...
//            if (MyApplication.getInstance().getIsLocationChina()) {
    ...
    private void onSelectStartAndEndTime() {
        ...
        timeSelectDialog.setDialogCallback(new TimeSelectDialog.DialogCallback() {
            ...
            public void EventOK(String sTime, String eTime) {
                ...
                startHour = sTime;
                endHour = eTime;
//                String startTimeGTM = TimeZoneUtil.GetGMTStrFromUTCStr(startTime, "");
//                String endTimeGTM = TimeZoneUtil.GetGMTStrFromUTCStr(endTime, "");
//
//                String[] start = startTimeGTM.split(" ");
//                startTime = start[0] + " " + sTime + ":00";
//
//                String[] end = endTimeGTM.split(" ");
//                endTime = end[0] + " " + eTime + ":00";
//
//                startTime = TimeZoneUtil.GetUTCStrFromGTMStr(startTime, "");
//                endTime = TimeZoneUtil.GetUTCStrFromGTMStr(endTime, "");
//
    ...
    public void getTrackListSuccess(TrackListResultBean trackListResultBean, boolean isResetData) {
        ...
        if (playDataForSegmented.size() >= 2) {
            ...
            mGoogleMap.addPolyline(new PolylineOptions().addAll(lineList).width(10).color(color));
        }
...
//                        rotateMarker.setToTop();
        ...
        if (wifiRoutePointListForSegmented.size() > 0) {
            ...
            for (int j = 0; j < wifiRoutePointListForSegmented.size(); j++) {
                ...
                if (routePointTmp.getType() == 2 || routePointTmp.getType() == 5) {
                    titleStringTmp = getTitleStringByType(routePointTmp.getType());
                    Marker wifiMarker = mGoogleMap.addMarker(new MarkerOptions().anchor(0.5f, 0.5f)
                            .position(routePointTmp.point)
                            .title(titleStringTmp)
                            .snippet(getTrackDetailInfo(routePointTmp))
                            .visible(isShowWifi)
        ...
        if (seekBarProcess != null) {
            ...
            if (playIndex <= playData.size() - 1) {
                ...
                if (carImageId == 0 || carImageId == 1 || carImageId == 2) {
                    lastPoint = playData.get(playIndex - 1).point;
                    mPlayRotate = GoogleMapUtils.getGoogleRotate(lastPoint, currentPoint.point);
                    playCurrrentMarker.setRotation(mPlayRotate + mGoogleMap.getCameraPosition().bearing);
                }
                if (isGetSuccess) {
                    isGetSuccess = false;
                    setAddress(currentPoint);
                ...
                tvLocationTime.setText(getString(R.string.txt_time) + DateUtils.timeConversionDate_two(String.valueOf(currentPoint.getTime())));
                tvSpeed.setText(getString(R.string.txt_speed) + ((double) playData.get(playIndex).getSpeed() / 10) + "km/h");
    ...
    //设置地址
    @SuppressLint("SetTextI18n")
    private void setAddress(RoutePointGoogle routePointGoogle) {
        double latForPoint = routePointGoogle.getLat();
        double lonForPoint = routePointGoogle.getLon();
