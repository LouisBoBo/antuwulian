...
import com.slxk.gpsantu.di.component.DaggerTrackBaiduComponent;
import com.slxk.gpsantu.mvp.contract.TrackBaiduContract;
import com.slxk.gpsantu.mvp.model.api.ModuleValueService;
import com.slxk.gpsantu.mvp.model.bean.AlertBean;
import com.slxk.gpsantu.mvp.model.bean.RoutePointBaidu;
...
import com.slxk.gpsantu.mvp.presenter.TrackBaiduPresenter;
...
public class TrackBaiduActivity extends BaseActivity<TrackBaiduPresenter> implements TrackBaiduContract.View,
    ...
    private BitmapDescriptor bitmapDescriptor;// 图标
...
//                    mapInfoWindow.tvMarkerSpeed.setText(getString(R.string.txt_speed_ex) + Utils.formatValue(Double.parseDouble(detailInfo[5]) * 3.6 / Integer.parseInt(detailInfo[4])) + "km/h");
//                } else {
...
//
    ...
    public void getTrackListSuccess(TrackListResultBean trackListResultBean, boolean isResetData) {
        ...
        for (int i = 0; i < trackListResultBean.getData().size(); i++) {
            ...
            RoutePointBaidu routePoint = new RoutePointBaidu(latLng, bean.getType(), bean.getTime(), bean.getSpeed(),
        ...
        if (playDataForSegmented.size() >= 2) {
            ...
            mBaiduMap.addOverlay(new PolylineOptions().points(lineList).width(8).color(color));
        }
...
//                        rotateMarker.setToTop();
        ...
        if (wifiRoutePointListForSegmented.size() > 0) {
            ...
            for (int j = 0; j < wifiRoutePointListForSegmented.size(); j++) {
                ...
                if (routePointTmp.getType() == 2 || routePointTmp.getType() == 5) {
                    titleStringTmp = getTitleStringByType(routePointTmp.getType());
                    Marker wifiMarker = (Marker) mBaiduMap.addOverlay(new MarkerOptions().anchor(0.5f, 0.5f)
                            .position(routePointTmp.point)
                            .title(titleStringTmp)
                            .visible(isShowWifi)
                    ...
                    Bundle bundle = new Bundle();
                    bundle.putString("snippet", getTrackDetailInfo(routePointTmp));
                    wifiMarker.setExtraInfo(bundle);
    ...
    private void setAddress(RoutePointBaidu routePointBaidu) {
        double latForPoint = routePointBaidu.getLat();
        double lonForPoint = routePointBaidu.getLon();
