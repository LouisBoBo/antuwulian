...
public class DeviceGroupResultBean extends BaseBean {
    ...
    public static class GaragesBean implements Parcelable {
        /**
        ...
        public int imei_count; // 设备总数
        public boolean isSelect = false; // 是否选中

        public GaragesBean(Parcel in) {
            sid = in.readString();
            sname = in.readString();
            imei_count = in.readInt();
            isSelect = in.readByte() != 0;
        }

        public static final Creator<GaragesBean> CREATOR = new Creator<GaragesBean>() {
            @Override
            public GaragesBean createFromParcel(Parcel in) {
                return new GaragesBean(in);
            }

            @Override
            public GaragesBean[] newArray(int size) {
                return new GaragesBean[size];
            }
        };
